...
Public Sub EncryptZip()
Dim A$, fname1$, fname2$, it&, x&, l%
Dim PbarVal As Single, IsZipped As Boolean
StartTimer = Timer
KillFiles
'----------------------- Get FileName -----------------------------
filename = gLocation & "IDI\Desktop\SEND\" & gParentCompany & ".zip"
...
'-------------------- Read & Write 16k at a time ------------------
       ...
       Kill gLocation & "IDI\Desktop\SEND\" & gParentCompany & ".zip"
End If
End Sub


Public Sub UnencryptZip()
Dim A$, fname1$, fname2$, it&, x&, l%
Dim PbarVal As Single, IsZipped As Boolean
StartTimer = Timer
KillFiles
'----------------------- Get FileName -----------------------------
filename = gLocation & "IDI\Desktop\MESSAGES\TEMP\" & gParentCompany & "_zip.sen"
...
l% = Len(filename)
If l% > 0 Then
        fname1$ = Left$(filename, InStr(1, filename, "_") - 1) & "." & Mid$(filename, InStr(1, filename, "_") + 1, InStr(1, filename, ".") - InStr(1, filename, "_") - 1)
...
Dummy1$ = "XCZVDSKL.BJK"
Dummy2$ = "KJBGFCDI.PFW"
If Dir(Dummy1$) <> "" Then Kill Dummy1$
If Dir(Dummy2$) <> "" Then Kill Dummy2$
